const DB = require('../../src/IdentityFieldDB')
const logkeeper = require('./logkeeper')
const WebSocket = require('ws')
const { ApiPromise, WsProvider, Keyring } = require('@polkadot/api')
const address = 'wsServer://kusama-rpc.polkadot.io'
const port = '9944'
const chainProvider = new WsProvider(`${address}:${port}`)
...
async function init () {
  ...
  const name = `identity-adaptor`
  const log = await logkeeper(name, logport)

  const wsServer = new WebSocket.Server({ port: PORT }, after)

  function after () {
    log({ type: 'chain', body: [`running on http://localhost:${wsServer.address().port}`] })
  }
...
function signAndSend (body, name, status) {
  ...
  const vaultAPI = await getVaultAPI({name, log})
  vaultAPI
